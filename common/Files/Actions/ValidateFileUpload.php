...
class ValidateFileUpload
{
    ...
    protected function validateAllowedExtensions(): string|null
    {
        ...
        if (empty($allowedExtensions) || !isset($this->fileData['extension'])) {
            return null;
        }

        if (!in_array($this->fileData['extension'], $allowedExtensions)) {
    ...
    protected function validateBlockedExtensions(): string|null
    {
        ...
        if (empty($blockedExtensions) || !isset($this->fileData['extension'])) {
            return null;
        }

        if (in_array($this->fileData['extension'], $blockedExtensions)) {
            return __('Files of this type are not allowed');
        }
